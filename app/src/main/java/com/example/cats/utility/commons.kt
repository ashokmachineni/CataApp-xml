package com.example.cats.utility
...
fun convertStringToList(value: String): ArrayList<String> {
    val token: TypeToken<ArrayList<String>> = object : TypeToken<ArrayList<String>>() {}
    if (SharedPrefHelper[value, ""] != null && !SharedPrefHelper[value, ""].isEmpty()) {
        return Gson().fromJson(SharedPrefHelper[value, ""], token.type)
    } else {
